...
import { removeMovieFromCompare, clearCompareMovies } from "@/store/compareMovieSlice";
          ...
          <motion.div
            ...
            <div className="w-full max-w-4xl mx-auto bg-gray-900/80 backdrop-blur-lg border-t-2 border-indigo-500 rounded-t-2xl shadow-2xl text-white">
              <div className="p-4 border-b border-gray-700 flex justify-between items-center">
                ...
                <button onClick={() => setIsOpen(false)} className="p-1 rounded-full text-gray-400 hover:bg-gray-700">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-4 max-h-64 overflow-y-auto">
                <ul className="space-y-2">
                  {compareMovies.map((movie) => (
                    <li key={movie.imdbID} className="flex items-center justify-between p-3 bg-gray-800/60 rounded-lg border border-transparent hover:border-purple-500/50 transition-colors duration-300 group">
                      <div>
              ...
              <div className="flex justify-end gap-4 p-4 border-t border-gray-700">
                <button
                  ...
                  className="px-4 py-2 bg-transparent border border-red-600 text-red-500 font-semibold rounded-lg hover:bg-red-600 hover:text-white transition-colors">
                  Clear List
                </button>
                <button
                  onClick={handleCompareClick}
                  className="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-500 transition-all">
                  Compare
                </button>
              </div>
            </div>
