...
import {
  Film, Search, Menu, Bell, User, Home, Star, Zap, Tv, Grid, X, Bookmark,
} from "lucide-react";
  ...
  return (
    <>
      <motion.header
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-red-600/5 via-purple-600/5 to-blue-600/5 opacity-80"></div>
        ...
        <div className="relative container mx-auto px-3 sm:px-4 md:px-6 h-14 sm:h-16 md:h-20 lg:h-24 flex items-center justify-between">
          <Link to="/movies" className="flex items-center gap-1 sm:gap-2 md:gap-4 hover:scale-105 transition-all duration-300 group flex-shrink-0">
            ...
            <div className="flex flex-col min-w-0">
              <motion.h1
                ...
                className="text-base sm:text-xl md:text-2xl lg:text-2xl xl:text-3xl font-black bg-gradient-to-r from-gray-900 via-red-600 to-purple-600 dark:from-white dark:via-red-200 dark:to-purple-200 bg-clip-text text-transparent tracking-tight truncate">
                CineVerse
              </motion.h1>
              <motion.p
                initial={{ opacity: 0.6 }}
                whileHover={{ opacity: 1 }}
                className="hidden sm:flex items-center gap-1 text-xs text-red-600 dark:text-red-400 font-semibold -mt-1 tracking-[0.2em] uppercase">
          ...
          <div className="hidden md:flex items-center flex-1 justify-start ml-4 lg:ml-8 xl:ml-12">
            <nav className="flex items-center gap-1 lg:gap-2">
              {[
                ...
                { name: "Analytics", path: "/analytics", icon: Grid },
              ].map(({ name, path, icon: Icon }) => (
                <Link
                  key={path}
                  to={path}
                  className={`relative group px-4 lg:px-6 py-3 transition-all duration-300 font-semibold tracking-wide ${
                    pathname === path ? "text-gray-900 dark:text-white" : "text-gray-700 dark:text-white/90 hover:text-gray-900 dark:hover:text-white"
                  }`}>
                  <span className="relative z-10 flex items-center gap-2">
                    <Icon className="w-4 h-4" />
                    {name}
                  </span>
                  <div className={`absolute inset-0 bg-gradient-to-r from-red-500/20 to-purple-500/20 rounded-xl transition-transform duration-300 shadow-lg ${pathname === path ? "scale-100" : "scale-0 group-hover:scale-100"}`}></div>
                  <div className={`absolute -bottom-1 left-0 w-full h-1 bg-gradient-to-r from-red-500 to-purple-500 rounded-full transition-transform duration-300 ${pathname === path ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}></div>
                </Link>
              ))}

              <Link
                to="/watchlist"
                className={`relative group px-4 lg:px-6 py-3 transition-all duration-300 font-semibold tracking-wide ${pathname === "/watchlist" ? "text-gray-900 dark:text-white" : "text-gray-700 dark:text-white/90 hover:text-gray-900 dark:hover:text-white"}`}>
                <span className="relative z-10 flex items-center gap-2">
              ...
              <div className="ml-4 px-4 py-2 bg-gradient-to-r from-amber-500/20 to-yellow-500/20 backdrop-blur-sm border border-amber-500/30 rounded-full">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-amber-600 dark:text-amber-300 font-bold text-sm tracking-wide">PREMIUM</span>
                </div>
              </div>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2 md:gap-3 flex-shrink-0">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="p-1.5 sm:p-2 md:p-3 text-gray-600 dark:text-white/80 hover:text-gray-800 dark:hover:text-white transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-md sm:rounded-lg md:rounded-xl backdrop-blur-sm border border-gray-200 dark:border-white/10">
            ...
            <Link to="/watchlist">
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                className="relative p-1.5 sm:p-2 md:p-3 text-gray-600 dark:text-white/80 hover:text-gray-800 dark:hover:text-white transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-md sm:rounded-lg md:rounded-xl backdrop-blur-sm border border-gray-200 dark:border-white/10">
                <Bookmark className="h-4 w-4 sm:h-4 sm:w-4 md:h-5 md:w-5" />
                {isHydrated && watchlistMovies.length > 0 && (
                  <div className="absolute -top-1 -right-1 w-2.5 h-2.5 md:w-3 md:h-3 bg-gradient-to-r from-red-500 to-pink-500 rounded-full border-2 border-white dark:border-black animate-pulse"></div>
                )}
              </motion.button>
            </Link>
            ...
            <motion.button
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              className="hidden sm:flex relative p-2 md:p-3 text-gray-600 dark:text-white/80 hover:text-gray-800 dark:hover:text-white transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg md:rounded-xl backdrop-blur-sm border border-gray-200 dark:border-white/10">
            ...
            <motion.div whileHover={{ scale: 1.05 }} className="hidden sm:block relative group">
              <Link to="/profile">
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-2 md:gap-3 p-1.5 sm:p-2 pr-2 sm:pr-3 md:pr-4 bg-gradient-to-r from-red-600/20 to-purple-600/20 hover:from-red-600/30 hover:to-purple-600/30 backdrop-blur-sm border border-red-500/30 dark:border-white/20 rounded-lg md:rounded-xl transition-all duration-300">
                  <div className="relative w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 bg-gradient-to-br from-red-500 to-purple-600 rounded-md sm:rounded-lg flex items-center justify-center overflow-hidden">
                    ...
                    <div className="absolute -bottom-0.5 -right-0.5 w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3 bg-green-400 border-2 border-white dark:border-black rounded-full"></div>
                  </div>
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              className="md:hidden p-1.5 sm:p-2 text-gray-600 dark:text-white/80 hover:text-gray-800 dark:hover:text-white transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-md sm:rounded-lg backdrop-blur-sm border border-gray-200 dark:border-white/10"
            ...
            <div className="mx-3 sm:mx-4 mt-2 bg-white/95 dark:bg-black/85 backdrop-blur-2xl border border-gray-200/50 dark:border-white/10 rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden">
              <div className="p-3 sm:p-4 space-y-1 sm:space-y-2">
                {[
                  ...
                  { name: "Actors", path: "/actors", icon: User },
                  { name: "Genres", path: "/genres", icon: Grid },
                  { name: "Watchlist", path: "/watchlist", icon: Bookmark, badge: isHydrated && watchlistMovies.length > 0 ? watchlistMovies.length : null },
                ].map((item, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      <Link
                        ...
                        className={`flex items-center gap-3 p-3 sm:p-4 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 ${isActive ? "text-white bg-gradient-to-r from-red-600 to-purple-600 shadow-lg" : "text-gray-700 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 hover:bg-white/50 dark:hover:bg-white/5"}`}>
              ...
              <div className="px-3 sm:px-4 pb-3 sm:pb-4 pt-2 border-t border-gray-200/50 dark:border-white/10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-gradient-to-r from-amber-500/20 to-yellow-500/20 backdrop-blur-sm border border-amber-500/30 rounded-full">
                    ...
                    <span className="text-amber-600 dark:text-amber-300 font-bold text-xs sm:text-sm">PREMIUM</span>
                  </div>
                  <Link to="/profile">
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      className="flex items-center gap-2 p-1.5 sm:p-2 bg-gradient-to-r from-red-600/20 to-purple-600/20 backdrop-blur-sm border border-red-500/30 dark:border-white/20 rounded-lg">
