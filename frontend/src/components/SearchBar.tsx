...
export interface SearchBarRef {
	focus: () => void;
	blur: () => void;
	// You might also expose internal state or other functions if needed
}
...
// Correctly exporting the forwardRef component
export const SearchBar = forwardRef<SearchBarRef, SearchBarProps>(
	function SearchBar( // Named function for better debugging
		{
			value,
			onChange,
		...
		const hasActiveFilters =
			year !== "" || genre !== "all" || searchType !== "both";
		...
		useImperativeHandle(ref, () => ({
			focus: () => {
				...
				setIsFocused(true);
				// Optionally show suggestions on focus if input is empty
				if (!value.trim()) {
		...
		const handleInputFocus = () => {
			setIsFocused(true);
			// Show suggestions only if there's no active filter expanded
		...
		const handleInputBlur = () => {
			setIsFocused(false);
			// Delay hiding suggestions to allow clicks on suggestion items before blur event fires
			setTimeout(() => setShowSuggestions(false), 150);
		};
		...
		// Handle search submission (e.g., on Enter key or explicit search button)
		const handleSearchSubmit = (searchValue: string) => {
			if (searchValue.trim()) {
				addToHistory(searchValue.trim());
			}
			// You might want to trigger the actual movie search here
			// This component only manages the search bar UI, the parent (MoviesPage)
			// typically handles the actual data fetching.
		};
		...
		const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
			if (e.key === "Enter" && value.trim()) {
				...
				setShowSuggestions(false);
				inputRef.current?.blur(); // Optionally blur after search
			} else if (e.key === "Escape") {
				setShowSuggestions(false);
				setIsFiltersExpanded(false); // Also close filters on escape
				inputRef.current?.blur();
			}
		};
				...
				<div className='relative'>
					...
					<motion.div
						className={`relative bg-white/95 dark:bg-black/70 backdrop-blur-3xl border-2 transition-all duration-500 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden ${
							isFocused
								? "border-red-500/60 shadow-red-500/25 dark:shadow-red-500/20" // Removed duplicate shadow-2xl
											...
											<div className='space-y-2 sm:space-y-3'>
												...
												<div className='relative'>
													<select
														...
														onChange={(e) =>
															onSearchTypeChange(
																e.target.value as "movie" | "actor" | "both"
															)
														}
					...
					<AnimatePresence>
						{showSuggestions && (
							<motion.div
								...
								// Fixed duplicate className by merging
																...
																<motion.button
																	...
																	<div className='flex-1 min-w-0'>
																		...
																		<div className='text-xs text-gray-500 dark:text-gray-400 mt-0.5'>
																			{searchHistoryUtils.formatRelativeTime(
																				item.timestamp
																			)}
																		</div>
																	</div>
															...
															{popularActors.map(
																(actor: any, index: number) => (
																	<motion.button
																		key={actor.text}
																		onClick={() =>
																			handleSuggestionClick(actor.text, "actor")
																		}
